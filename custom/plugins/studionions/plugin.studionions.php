...
switch ($e->name) {
	...
	case "OnRichTextEditorRegister":
		//$modx->config['which_editor'] = $editor = "none";
		break;
	case "OnDocFormPrerender":
		global $content;
		if(empty ($content['pub_date'])):
			$timeNow = $_SERVER['REQUEST_TIME'] + $this->config['server_offset_time'];
			$content['pub_date'] = $timeNow;//date('d-m-Y H:i:00', time());
		endif;
		break;
	case "OnBeforeDocFormSave":
		global $content;
		PluginEvolution::onBeforeDocSave($modx, $params);
		break;
	case "OnDocFormSave":
	case "OnDocDuplicate":
		//PluginEvolution::createDocFolders($modx, $params);
		//PluginEvolution::getYoutubeThumbnails($modx, $params);
		break;
